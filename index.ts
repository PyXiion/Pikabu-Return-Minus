  ...
  export class Post extends RatingObject {
    ...
    private parseData(dataArr: any) {
      for (let data of dataArr) {
        if ((data.type as string).includes("v")) {
          // v means video (maybe)
          data = data.data;
          let urls = [];
...
namespace RPM {
  export const STYLE = `.rpm-modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
...
`;
  ...
  export namespace Service {
    ...
    async function workQueue(sleepTime: number = 0) {
      ...
      const requestsToProcess = Array.from(userInfoRequestQueue.keys()).slice(
        0,
      ...
      try {
        ...
        requestsToProcess.forEach((id) => {
          ...
          if (userRequests) {
            ...
            if (info) userRequests.forEach((req) => req.callback(info));
          }
  ...
  export namespace Nodes {
    ...
    export function createUserVoteReasonContainer(text: string) {
      const reasonElem = RPM.createElementWithClass(
        "div",
        "rpm-vote-reason-container"
      ) as HTMLDivElement;

      const reasonTitleElem = RPM.createElementWithClass(
        "h4",
        "rpm-vote-reason-title"
      ) as HTMLParagraphElement;
      reasonTitleElem.textContent = "Заметка RPM";

      const reasonTextElem = RPM.createElementWithClass(
        "p",
        "rpm-vote-reason"
      ) as HTMLParagraphElement;
    ...
    function attachVoteListeners(
      ...
      const handleVote = (vote: number) =>
        UserRating.voteCallback(elem, uid, vote);
    ...
    namespace UserRating {
      ...
      function updateSpan(
        ...
        const elem = parent.querySelector(selector) as HTMLSpanElement;
        elem.style.display = "";
      ...
      export async function voteCallback(
        ...
        if (vote === -1) {
          UserRating.showReasonDialog(uid, -1);
        } else {
      ...
      /**
       * Creates and shows a dialog for selecting a reason with radio buttons.
       * @param {number} uid - The user ID.
       * @param {number} vote - The vote value (-1).
       */
      export async function showReasonDialog(uid: number, vote: number) {
        ...
        header.innerText = "Выберите причину минуса";
        ...
        const cancelButton = createElementWithClass(
          "button",
          "rpm-modal-button"
        );
        ...
        submitButton.addEventListener("click", async () => {
          const selectedReason = document.querySelector<HTMLInputElement>(
...
namespace Gollum {
  ...
  async function getUserId(userName: string) {
    ...
    const request = new HttpRequest(
      `https://gollum.space/user/${userName.replace(".", "_")}-summary`,
      "GET",
      "text"
    );
    const response = await request.executeAsync();
  ...
  export const createLastCommentsSection =
    LastComments.createLastCommentsSection;
...
const currentStoryId = parseInt(
  ["0", ...window.location.href.split("_")].pop()
);
  ...
  GM_config.init({
    ...
    fields: {
      ...
      minStoryRating: {
        ...
        default: 100,
        label:
          "Посты с рейтингом ниже указанного будут удаляться из ленты. Вы сможете увидеть удалённые посты в списке просмотренных.",
      },
      ratingBar: {
        type: "checkbox",
        default: true,
      ...
      rpmStoryVoteReason: {
        ...
        default: true,
        label:
          "Показывать причину оценки (если таковая имеется) в начале поста.",
      },
      registerRpm: {
        type: "button",
        label:
        ...
        async click() {
          ...
          if (uuid === null || uuid === undefined || uuid === "") {
            ...
            sendNotification(
              ...
              "Вы не можете зарегистрироватся ещё раз."
            );
          }
        },
      },
      ...
      miniProfileAutoloadPikabuCommentCount: {
        ...
        default: 1,
        label:
          "Сколько комментариев загрузить без нажатия на кнопку Загрузить.",
      },
    ...
    events: {
      init() {
        ...
        formats.formatStoryMinuses = makeEval(
          "story",
          this.get("minusesPattern") as string,
          (x: any) => x.minuses
        );
        formats.formatCommentMinuses = makeEval(
          "comment",
          this.get("minusesCommentPattern") as string,
          (x: any) => x.minuses
        );
        formats.formatOwnComment = makeEval(
          "comment",
          this.get("ownCommentPattern") as string,
          (x: any) =>
            x.pluses == 0 && x.minuses == 0 ? 0 : `${x.pluses}/${x.minuses}`
        );

        enableFilters = new RegExp(
          this.get("filteringPageRegex") as string
...
const waitConfig = () =>
  new Promise<void>((resolve) => {
    let isInit = () =>
      setTimeout(() => (isConfigInit ? resolve() : isInit()), 1);
    isInit();
  });
...
function processComment(comment: Pikabu.Comment | CommentData) {
  ...
  if (!userElem || !ratingDown) {
    ...
    info(
      "У комментария",
      comment.id,
      " нет юзера или кнопок рейтинга, кэширую его"
    );
...
async function processStoryComments(
  ...
  if (storyData.hasMoreComments) {
    ...
    sendNotification(
      "Return Pikabu Minus",
      "Все рейтинги комментариев загружены!"
    );
  }
}
...
async function checkStoryLinks(story: HTMLDivElement) {
...
async function processStoryRpm(story: HTMLDivElement) {
  ...
  else footerElem.parentElement.insertBefore(elem, footerElem);
...
function processPostVideos(
  ...
  function addUrlToPlayer(videoBlock: HTMLElement, urls: string[]) {
    ...
    if (urlListElem.hasChildNodes())
      videoBlock.parentElement.insertBefore(
        urlListElem,
        videoBlock.nextSibling
      );
    else urlListElem.remove();
  }
...
function addVideoDownloadButtons(postId: number, url: string) {
  const videoControls: HTMLDivElement[] = Array.from(
    document.querySelectorAll(
      `.story[data-story-id="${postId}"] .player__controls`
    )
  );
...
function appendCustomThemesUi(themePicker: HTMLDivElement) {
  ...
  // Dark mode buttons
  const darkModeLabel = themePicker.querySelector(
    ".theme-picker__option_dark"
  ) as HTMLLabelElement;
  ...
  const basicThemes = Array.from(buttonsContainer.children).map((x) =>
  ...
  function createTheme(
    ...
    if (!isBasic) themeButton.classList.add("rpm-theme-picker");
    if (name !== null) themeButton.setAttribute("data-name", name);
...
async function requestCsrfToken() {
  ...
  const appConfig = JSON.parse(
    document.querySelector(".app__config").textContent
  );
  ...
  function gollumIntegration() {
    ...
    if (worked) {
      const poweredElem = RPM.Nodes.createPoweredNote(
        `Данные получены с <a href="https://gollum.space/user/${userName.replace(
          ".",
          "_"
        )}-summary" target="_blank">gollum.space</a>`
      );
...
namespace FeedbackManager {
  export async function init() {
    ...
    const isToday =
      settings.lastCheckDate.toDateString() === new Date().toDateString();
  ...
  function updateSettings(settings: FeedbackSettings) {
    settings.saved = settings.saved.filter(
      (fb) => !settings.completed.includes(fb[1].id)
    );
